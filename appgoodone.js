...
function resetFilters() {
  document.querySelectorAll('.left-nav input[type="radio"]').forEach(el => el.checked = false);
  document.getElementById("selectionSummary").innerText = '';
  currentPage = 0;
  renderEvents();
}
    ...
    for (let col = 0; col < 3; col++) {
      ...
      if (pageEvents[index]) {
        ...
        td.innerHTML = `
          ...
          <button class="button" onclick="location.href='${event.detail}'">View Details / Get Quotation</button>
        `;
      } else {
...
const sizeRadios = document.getElementsByName("groupsize");
...
function getSelectedFilters() {
  let selectedCategory = [...categoryRadios].find(r => r.checked)?.value;
  if (selectedCategory === "other") {
    selectedCategory = otherCategoryInput.value.toLowerCase().trim();
  }

  const selectedSize = parseInt([...sizeRadios].find(r => r.checked)?.value || "0", 10);
  const selectedPayment = [...paymentRadios].find(r => r.checked)?.value;

  return { selectedCategory, selectedSize, selectedPayment };
}

// Render Filtered Events
function renderEvents() {
  const { selectedCategory, selectedSize, selectedPayment } = getSelectedFilters();
  eventContainer.innerHTML = "";

  // Filter events
  const filtered = events.filter(evt => {
    const categoryMatch = evt.category.toLowerCase() === selectedCategory;
    const sizeMatch = evt.capacity <= selectedSize;
    const paymentMatch = evt.payment.includes(selectedPayment);
    return categoryMatch && sizeMatch && paymentMatch;
  });

  // Display selected filters
  filterSummary.innerHTML = `
    <strong>Selected Category:</strong> ${selectedCategory} <br>
    <strong>Group Size:</strong> ≤ ${selectedSize} people <br>
    <strong>Payment Mode:</strong> ${selectedPayment}
  `;
...
// Trigger rendering on filter change
document.querySelector(".left-nav").addEventListener("change", renderEvents);
document.getElementById("applyFiltersBtn").addEventListener("click", renderEvents);
...
function applyFilters() {
 ...
 // `;
  ...
  filteredEvents = events.filter(event => {
    ...
    const matchPayment = payment ? event.payment === payment : true;
...
function renderEvents() {
  ...
  const sorted = [...filteredEvents].sort((a, b) => a.name.localeCompare(b.name));
  const start = currentPage * itemsPerPage;
  const pageEvents = sorted.slice(start, start + itemsPerPage);
  ...
  for (let row = 0; row < 2; row++) {
    ...
    for (let col = 0; col < 3; col++) {
      ...
      if (pageEvents[index]) {
        const e = pageEvents[index];
        td.innerHTML = `
          <img src="${e.image}" alt="${e.name}" />
          <p><strong>${e.name}</strong></p>
          <p>Capacity: ${e.capacity}</p>
          <p>Venue: ${e.venue}</p>
          <button class="button" onclick="openEventDetail('${e.name}', '${e.image}', '${e.capacity}', '${e.venue}')">
...
function openEventDetail(name, image, capacity, venue) {
  const url = new URL('event-detail.html', window.location.href);
